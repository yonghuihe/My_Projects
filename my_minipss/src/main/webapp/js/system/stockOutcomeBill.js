$(function(){
	...
	$("#edit_table_body").on("click",".searchproduct",function(){//监听放大镜按钮
		...
		$.dialog.open("/product_view.action",{
			...
			close:function(){
				...
				tr.find("[tag='salePrice']").val(productVal.salePrice)
	...
	}).on("blur","[tag='salePrice'],[tag='number']",function(){//监听tag="salePrice"和tag="number"
		var tr = $(this).closest("tr");
		var constPriceVal = tr.find("[tag='salePrice']").val();
	...
	$(".btn_submit").click(function(){
		...
		$.each($("#edit_table_body tr"),function(index,item){
			$(item).find("[tag='pid']").prop("name","stockOutcomeBill.stockOutcomeBillItem[" + index + "].product.id");
			$(item).find("[tag='salePrice']").prop("name","stockOutcomeBill.stockOutcomeBillItem[" + index + "].salePrice");
			$(item).find("[tag='number']").prop("name","stockOutcomeBill.stockOutcomeBillItem[" + index + "].number");
			$(item).find("[tag='remark']").prop("name","stockOutcomeBill.stockOutcomeBillItem[" + index + "].remark");
		})
		$("#editForm").submit();
	});
