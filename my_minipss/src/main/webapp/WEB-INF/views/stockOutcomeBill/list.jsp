				...
				<div class="ui_text_indent">
					<div id="box_border">
						...
						<div id="box_center">
							...
							客户：
							<s:select list="#clients" cssClass="ui_select01" name="queryObject.clientId" listKey="id" listValue="name" headerKey="-1" headerValue="---请选择客户---"/>
							状态：
						...
						<div id="box_bottom">
							...
							<input type="button" value="新增" class="ui_input_btn01 btn_input" data-href='<s:url action="stockOutcomeBill_input" />'/> 
						</div>
					</div>
				</div>
			</div>
						...
						<tbody>
							<s:iterator value="#result.result" var="item">
								<tr>
									...
									<td><s:property value="#item.client.name"/></td>
									...
									<td>
										<s:if test="#item.status == 0">
											<s:url action="stockOutcomeBill_audit" var="#auditUrl">
												<s:param name="stockOutcomeBill.id" value="#item.id"/>
											</s:url>
											<a href="javascript:;" class="btn_audit" data-url='<s:property value="#auditUrl"/>'>审核</a>
											<s:a action="stockOutcomeBill_input"><s:param name="stockOutcomeBill.id" value="#item.id"/>编辑</s:a>
											<s:url action="stockOutcomeBill_delete" var="#delUrl">
												<s:param name="stockOutcomeBill.id" value="#item.id"/>
											</s:url>
											<a href="javascript:;" class="btn_delete" data-url='<s:property value="#delUrl"/>'>删除</a>
										</s:if>
										<s:elseif test="#item.status == 1">
											<s:a action="stockOutcomeBill_view"><s:param name="stockOutcomeBill.id" value="#item.id"/>查看</s:a>
										</s:elseif>
									</td>
								</tr>
							</s:iterator>
