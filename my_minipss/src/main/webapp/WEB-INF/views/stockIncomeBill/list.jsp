...
<s:debug></s:debug>
	...
	<s:form id="searchForm" action="stockIncomeBill" method="post">
		<div id="container">
				...
				<div class="ui_text_indent">
					<div id="box_border">
						...
						<div id="box_bottom">
							...
							<input type="button" value="新增" class="ui_input_btn01 btn_input" data-href='<s:url action="stockIncomeBill_input" />'/> 
						</div>
					</div>
				</div>
			</div>
						...
						<tbody>
							<s:iterator value="#result.result" var="item">
								<tr>
									...
									<td>
										<s:if test="#item.status == 0">
											<s:url action="stockIncomeBill_audit" var="#auditUrl">
												<s:param name="stockIncomeBill.id" value="#item.id"/>
											</s:url>
											<a href="javascript:;" class="btn_audit" data-url='<s:property value="#auditUrl"/>'>审核</a>
											<s:a action="stockIncomeBill_input"><s:param name="stockIncomeBill.id" value="#item.id"/>编辑</s:a>
											<s:url action="stockIncomeBill_delete" var="#delUrl">
												<s:param name="stockIncomeBill.id" value="#item.id"/>
											</s:url>
											<a href="javascript:;" class="btn_delete" data-url='<s:property value="#delUrl"/>'>删除</a>
										</s:if>
										<s:elseif test="#item.status == 1">
											<s:a action="stockIncomeBill_view"><s:param name="stockIncomeBill.id" value="#item.id"/>查看</s:a>
										</s:elseif>
									</td>
								</tr>
							</s:iterator>
