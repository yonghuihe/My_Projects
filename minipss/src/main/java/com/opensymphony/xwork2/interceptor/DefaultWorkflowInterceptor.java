...
public class DefaultWorkflowInterceptor extends MethodFilterInterceptor {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(DefaultWorkflowInterceptor.class);
		...
		if (action instanceof ValidationAware) {
			...
			if (validationAwareAction.hasErrors()) {
				if (LOG.isDebugEnabled()) {
					LOG.debug(
							"Errors on action [#0], returning result name [#1]",
							validationAwareAction, inputResultName);
				}

				String resultName = inputResultName;
				resultName = processValidationWorkflowAware(action, resultName);
				resultName = processInputConfig(action, invocation.getProxy()
						.getMethod(), resultName);
	 ...
	 */
	private String processValidationWorkflowAware(final Object action,
			final String currentResultName) {
		...
		if (action instanceof ValidationWorkflowAware) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug(
						"Changing result name from [#0] to [#1] because of processing [#2] interface applied to [#3]",
	...
	protected String processInputConfig(final Object action,
			final String method, final String currentResultName)
			throws Exception {
		String resultName = currentResultName;
		InputConfig annotation = action.getClass()
				.getMethod(method, EMPTY_CLASS_ARRAY)
		...
		if (annotation != null) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug(
						"Changing result name from [#0] to [#1] because of processing annotation [#2] on action [#3]",
						currentResultName, resultName,
	 ...
	 */
	protected String processValidationErrorAware(final Object action,
			final String currentResultName) {
		String resultName = currentResultName;
		if (action instanceof ValidationErrorAware) {
			resultName = ((ValidationErrorAware) action)
					.actionErrorOccurred(currentResultName);
			if (LOG.isDebugEnabled()) {
				LOG.debug(
						"Changing result name from [#0] to [#1] because of processing interface [#2] on action [#3]",
						currentResultName, resultName,
